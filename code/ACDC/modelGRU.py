...
class TransGRU(nn.Module):
    def __init__(self, input_channels, hidden_channels,
        ...
        self.bias = bias
        self.all_layers = []
        # self.query_conv = nn.Conv2d(hidden_channels[-1], hidden_channels[-1], 1)
        # self.key_conv = nn.Conv2d(hidden_channels[-1], hidden_channels[-1], 1)
        # self.value_conv = nn.Conv2d(hidden_channels[-1], hidden_channels[-1], 1)
        # self.gate_fc = nn.Sequential(
        #     nn.Conv2d(4 * hidden_channels[-1], 2 * hidden_channels[-1], 1),
        #     nn.GLU(dim=1)
        # )
    ...
    def forward(self, x):
        ...
        for step in range(steps):
            ...
            for layer in range(self.num_layers):
                ...
                input, r, z, xr, xz, new_c, new_m, gc, gm,o= getattr(self, name)(input, h,c,m)  # forward propagation call
                internal_state[layer] = (input ,new_c, new_m)
            outputs.append(input)
        output = outputs[-1]
        query, keys, values = self.get_QKV(output, r, z, xr, xz, new_c, new_m, gc, gm, o)
        query, keys, values = query.view(bsize, -1, height * width), keys.view(bsize, -1, height * width), values.view(
...
class SAGRU(nn.Module):
    def __init__(self, input_channels=64, hidden_channels=[64], kernel_size=5, bias=True, num_classes=4,
                 attenion_size=224 * 224, encoder=None):
        super(SAGRU, self).__init__()
        self.encoder = encoder
        self.grulayer = TransGRULayer(input_channels, hidden_channels, kernel_size, bias, num_classes, attenion_size)

    def forward(self, x1, x2, x3):
        x1 = self.encoder(x1)
        x2 = self.encoder(x2)
        x3 = self.encoder(x3)
        y = self.grulayer(x1, x2, x3)
        return y