...
from lib.networks_synapse import MERIT_Parallel_SAGRU
...
parser.add_argument('--model_path', type=str, required=True, help='path to model weights (.pth) to test')
...
if(args.num_classes == 14):
    ...
    classes = ['aorta', 'gallbladder', 'kidney(L)', 'kidney(R)', 'liver', 'pancreas', 'spleen', 'stomach']
...
if __name__ == "__main__":
    ...
    # experiment name (kept consistent with train script naming schema)
    args.exp = 'MERIT_Parallel_Small_loss_MUTATION_w3_7_' + dataset_name + str(args.img_size)

    net = MERIT_Parallel_SAGRU(n_class=args.num_classes, img_size_s1=(args.img_size,args.img_size), img_size_s2=(224,224), model_scale='small', decoder_aggregation='additive', interpolation='bilinear').cuda()

    # load model weights from runtime argument
    ...
    if args.is_savenii:
        # save predictions next to the model file by default
        default_save_root = os.path.join(os.path.dirname(snapshot), "predictions")
        args.test_save_dir = args.test_save_dir or default_save_root
