...
def trainer_synapse(args, model, snapshot_path):
    ...
    db_train = Synapse_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train", nclass=args.num_classes,
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size], noise=getattr(args, 'noise', 'none'))]))
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if (not saved_noisy_example) and getattr(args, 'noise', 'none') != 'none':
                try:
                    case_name = sampled_batch.get('case_name', ['sample'])[0]
                except Exception:
                    case_name = 'sample'
                save_dir = os.path.join(snapshot_path, 'noisy_examples')
                os.makedirs(save_dir, exist_ok=True)
                img = image_batch[0].clone()
                imin = img.min()
                imax = img.max()
                img = img - imin
                if (imax - imin) > 0:
                    img = img / (imax - imin)
                save_path = os.path.join(save_dir, f"{case_name}_noise_{args.noise}.png")
                save_image(img, save_path)
                logging.info(f"Saved noisy example to {save_path}")
                saved_noisy_example = True
    ...
    writer.close()
    # After training, evaluate the best checkpoint on the test set with Dice and HD95
    try:
        best_path = os.path.join(snapshot_path, 'best.pth')
        if not os.path.exists(best_path):
            best_path = os.path.join(snapshot_path, 'last.pth')
        state = torch.load(best_path)
        model.load_state_dict(state)
        logging.info(f"Loaded best model from {best_path} for final evaluation")
        ...
        metric_mean = metric_sum / len(db_test)
        mean_dice = float(np.mean(metric_mean, axis=0)[0])
        mean_hd95 = float(np.mean(metric_mean, axis=0)[1])
        logging.info(f"Final evaluation (best model): mean_dice {mean_dice:.6f} mean_hd95 {mean_hd95:.6f}")
